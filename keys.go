...
type idleKeyMap struct {
	Enter, Paste, Clear, Quit, ThemePicker key.Binding
}

type playingKeyMap struct {
	PauseToggle, Stop, Quit key.Binding
}
...
func (k idleKeyMap) ShortHelp() []key.Binding {
	return []key.Binding{k.Enter, k.Paste, k.Clear, k.Quit, k.ThemePicker}
}

func (k playingKeyMap) ShortHelp() []key.Binding {
	return []key.Binding{k.PauseToggle, k.Stop, k.Quit}
}
